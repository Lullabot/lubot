...
if (process.env.LUBOT_IRC_NICK_PW === 'undefined') {
...
bot.irc.once("registered", function () {
...
bot.helpers.utils = {
  ...
  startsWith: function (token, text) {
    ...
    if (start === token) {
      var newText = text.slice(token.length);
      if (newText.length > 0) {
        return newText;
      }
    }
  ...
  startsWithBot: function (token, text) {
    var botText = this.startsBot(text);
    if (botText !== false) {
  ...
  removeKV: function (key, collection_name) {
    if (typeof key === 'string') {
      this.mongoClient().connect(config.mongoUrl, function (err, db) {
        ...
        var collection = db.collection(collection_n);
        collection.remove({key: key}, {w: 1}, function (err) {
          if (err) { throw err; }
          db.close();
        });
      });
  ...
  removeFromCollection: function (collection_name, search) {
    if (typeof collection_name === 'string' && typeof search === 'object') {
      this.mongoClient().connect(config.mongoUrl, function (err, db) {
        if (err) { throw err; }
        var collection = db.collection(config.mongoPrefix + collection_name);
        collection.remove(search, {w: 1}, function (err) {
          if (err) { throw err; }
          db.close();
        });
      });
...
bot.helpers.irc = {
  ...
  'channelUsers': function (channel) {
    if (channel === "undefined"
        && bot.irc.chans === "undefined"
        && bot.irc.chans[channel] === "undefined"
        && bot.irc.chans[channel].users === "undefined") {
      return {};
    }
...
bot.brain.mongoClient().connect(config.mongoUrl, function (err, db) {
  if (err) { throw err; }
  db.ensureIndex(config.mongoPrefix + 'logs', {text: "text"});
  bot.irc.addListener("message#", function (nick, to, text) {
